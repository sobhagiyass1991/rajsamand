...
def list(request):
    news = NewsList.objects.filter(created_by=request.user).order_by('-id')
    context = {
        "news" : news
    }
...
def getadd(request):
    ...
    return render(request, "partials/addnewsform.html", context)
    ...
    if request.headers.get('HX-Request') and request.method == "POST":
        ...
        if form.is_valid():
            ...
            if request.headers.get('HX-Request') and blog:
              ...
              myblogs = NewsList.objects.filter(created_by=request.user).order_by('-id')
              ...
              response = render(request, "partials/mynews.html", contextt)
              response["HX-Trigger"] = json.dumps({"mysuccess": "डेटा सृजन में सेव हो गया!"})
              response.write('<div id="addnewsform" hx-swap-oob="true"></div>')          
              return response 
...
def getedit(request, id):
    if request.htmx and request.method == "GET":
       ...
       id = NewsList.objects.get(id=id)
       print(f"yah rahi aayi edit id {id}")
       #getid = ServiceProvider.objects.get('id')
       blogs = NewsList.objects.all()
       editform = NewsForm(instance=id)
       return render(request, "partials/editnews.html", { 'editform' : editform }  )
...
def saveedit(request, id):
    ...
    if request.htmx and request.method == "POST":
        ...
        form = NewsForm(request.POST, instance=id)
        print(f"form is valid {form} and errors are {form.errors}")
        if form.is_valid():
            form = NewsForm(request.POST, instance = id)
            print(3)
            ...
            #messages.success(request, "your data is saved")
            print(f" here is user id {request.user}")
            mydonors = NewsList.objects.filter(created_by=request.user).order_by('-id')
            #mydonors = Donor.objects.all().order_by('-id')
            ...
            response = render(request, "partials/mynews.html", contextt)
            response["HX-Trigger"] = json.dumps({"mysuccess": "डेटा सृजन में सेव हो गया!"})
            response.write('<div id="editnews" hx-swap-oob="true"></div>')          
            return response 
